...
use Swagger\Client\Model\Peticao;
...
class ProtocolosApi
{
    ...
    protected function createProcessProtocoloRequest(Protocolo $protocolo, $pje_auth_id)
    {
        ...
        if ($protocolo->getPeticao() === null) {
            ...
            if ($protocolo->getPeticao()->getArquivo() === null) {
                throw new \InvalidArgumentException(
                    "O parametro 'peticao.arquivo' é obrigatório!"
                );
            }

            if ($protocolo->getPeticao()->getTipoDocumento() === null) {
                throw new \InvalidArgumentException(
                    "O parametro 'peticao.tipo_documento' é obrigatório!"
                );
            }

            if ($protocolo->getPeticao()->getDescricaoDocumento() === null) {
                throw new \InvalidArgumentException(
                    "O parametro 'peticao.descricao_documento' é obrigatório!"
                );
            }
        ...
        if (isset($_tempBody)) {
            ...
            if ($multipart) {
                ...
                foreach ($formParams as $formParamName => $formParamValue) {
                    ...
                    {
                        $multipartContents = array_merge($multipartContents, $this->serializeDocumentoObjectRequest($formParamValue));
                    }
                }

                if ($protocolo->getPeticao() !== null) {
                    $multipartContents = array_merge($multipartContents, $this->serializePeticaoObjectRequest($protocolo->getPeticao()));
                }
    ...
    protected function serializePeticaoObjectRequest(Peticao $peticao)
    {
        $multipartContents = [];

        $multipartContents[] = [
            'name' => "peticao[arquivo]",
            'contents' => $peticao->getArquivo()
        ];
    ...
    protected function serializeDocumentoObjectRequest($formParam)
    {
        $index = 0;
        $multipartContents = [];
